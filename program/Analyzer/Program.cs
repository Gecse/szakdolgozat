...
namespace Analyzer
{
    class Program
    {
        static void Main()
        {
            var implementationFilePath = @"..\Analyzer.Exercise\AnalyzerExercise.cs";
            ...
            {
                if (e.StackTrace.Contains("ThrowMoreThanOneMatchException"))
                {
                    Console.WriteLine("Use a single method instead of multiple");
                }
            }
        }
        ...
        private static bool CanUseExpressionBody(SyntaxNode? root)
        {
            var greetingMethod = root.DescendantNodes()
                ...
                .Single(methodDeclaration => methodDeclaration.Identifier.Text == "Hello");
        ...
        private static bool UsesMethodOverloading(SyntaxNode? root)
        {
            return root
                .DescendantNodes()
                .OfType<MethodDeclarationSyntax>()
                .Count(methodDeclaration => methodDeclaration.Identifier.Text == "Hello") > 1;
        }
    }
}